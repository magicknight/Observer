...
def train(classifier_file, training_path, classifier_name, dim_x, dim_z, orientations, pixels_per_cell, cells_per_block,
          scan_window_size, weight_values, use_distance_transform=True, distance_threshold=0.3,
          re_train=False, online_training=False, verbose=False):
    # get progress bar for display progress
    """


    :type cells_per_block: object
    ...
    :param weight_values: 
    :param use_distance_transform: 
    :param distance_threshold: 
    :param re_train: 
    :param online_training: 
    :param verbose: 
    :return: 
    """
            ...
            for root, dirs, files in walk(training_path):
                for file_name in files:
                    if use_distance_transform:
                        training_sample, training_label, dummy, dummy = \
                            distance_transform_samples(join(root, file_name),
                                                       dim_x, dim_z,
                        ...
                        training_sample, training_label, dummy, dummy = get_hog_samples(join(root, file_name), dim_x, dim_z,
                                                                                 orientations, pixels_per_cell,
                                                                                 cells_per_block, scan_window_size,
                                                                                 training=True, verbose=verbose)
                    if online_training:
                        n_positive = np.count_nonzero(training_label)
                        sample_weight = [weight_values[0]] * (len(training_label) - n_positive) + [weight_values[
                                                                                                       1]] * n_positive
                        if file_count == 0:
                    ...
                    if file_count / number_of_total_files_over_20 == float(file_count) / float(
                            number_of_total_files_over_20):
            ...
            for file_name in files:
                ...
                if online_training:
                    ...
                    sample_weight = [weight_values[0]] * (len(training_label) - n_positive) + [weight_values[
                                                                                                   1]] * n_positive
                    if file_count == 0:
                        #print training_sample, training_label, np.unique(training_label), sample_weight
                        clf.partial_fit(training_sample, training_label, classes=np.unique(training_label),
                                        sample_weight=sample_weight)
                        print 'training labels are', np.unique(training_label)
                    else:
